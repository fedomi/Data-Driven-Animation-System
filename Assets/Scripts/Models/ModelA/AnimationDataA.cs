...
public class AnimationDataA : MonoBehaviour
{
    #region Attributes
    ...
    private void ExportData() {
        ...
        {
            foreach (MotionStateA m in msl)
            {
                //string line = m.GetP() + "," + m.GetQ() + "," + MotionStateA.Vector3ListToString(m.GetJ()) + "," + MotionStateA.Vector3ListToString(m.GetH());
            ...
            new System.IO.StreamWriter(@"Assets\AnimationFiles\" + TrajectoriesFile))
        {
            foreach (MotionStateA m in msl)
            {
                string line = MotionStateA.ExportVector3ListAsString(m.GetTP()).Substring(1);
                file.WriteLine(line);
