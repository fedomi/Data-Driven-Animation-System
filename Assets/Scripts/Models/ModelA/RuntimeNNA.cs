...
public class RuntimeNNA : MonoBehaviour
{

    #region Attributes

    private Body b;
    private MotionStateA m;
    private List<MotionStateA> msl;
    ...
    private static string filePythonNamePath = "F:/ProyectosUnity/VanillaMotionMatching/VanillaMotionMatching/Assets/Scripts/Python/TrainedNNA.py";
    ...
    {/*
        if (currentTime > inputTimer)
        {
            ...
            MotionStateA m = new MotionStateA(numbers);

            Debug.Log("Applying pose to " + m.GetJ().Count + " joints.");
            b.ApplyPose(m.GetJ(), m.GetH());
            //Debug.Log("Receiving A: " + received_data);
            //Debug.Log("Applying pose: " + MotionStateA.ExportVector3ListAsString(m.GetJ()));
    ...
    private void FixedUpdate()
    {
        //if (Input.GetKeyDown(KeyCode.Space)) {
            ...
            /*
            string received_data = MotionStateA.ExportVector3ListAsString(msl[(int)(currentFrame+1)].GetJ());
            //Debug.Log("Received: " + received_data);
            string[] numbers = received_data.Split(',');
    ...
    private void DrawDebugLines(MotionStateA m)
    {
        ...
        Debug.Log(MotionStateA.Vector3ListToString(tps));
    ...
    private void ImportData()
    {
        ...
        using (StreamReader file = new StreamReader(@"Assets\AnimationFiles\" + DataFile))
        {
            ...
            while ((ln = file.ReadLine()) != null)
            {
                ...
                /*if(line.Length == 58)
                {
                    MotionStateA ms = new MotionStateA(line);
