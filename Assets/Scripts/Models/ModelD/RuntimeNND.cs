...
public class RuntimeNND : MonoBehaviour
{
    ...
    private string DataFile = "MotionData_015.txt";
    ...
    private static string filePythonNamePath = "F:/ProyectosUnity/VanillaMotionMatching/VanillaMotionMatching/Assets/Scripts/Python/TrainedNND.py";
    ...
    {/*
        if (currentTime > inputTimer)
        {
            ...
            MotionStateD m = new MotionStateD(numbers);

            Debug.Log("Applying pose to " + m.GetJ().Count + " joints.");
            b.ApplyPose(m.GetJ(), m.GetH());
            //Debug.Log("Receiving A: " + received_data);
            //Debug.Log("Applying pose: " + MotionStateD.ExportVector3ListAsString(m.GetJ()));
     ...
     private void FixedUpdate()
    {
        //if (Input.GetKeyDown(KeyCode.Space)) {
            ...
            /*
            string received_data = MotionStateD.ExportVector3ListAsString(msl[(int)(currentFrame+1)].GetJ());
            //Debug.Log("Received: " + received_data);
            string[] numbers = received_data.Split(',');
   ...
   // b.ApplyPose(m.GetJ());
    ...
    private void FixedUpdate()
    {
        ...
        if (A)
        {
            ...
            string[] numbers = received_data.Split(' ');
            //MotionStateD m = new MotionStateD(numbers);
            MotionStateD m = new MotionStateD();
            m.AddPose(numbers);
            ...
            m.SetTP(msl[(int)currentFrame + 1].GetTP());
            m.SetPhase(msl[(int)currentFrame + 1].GetPhase());
            //Debug.Log("Pose: " + MotionStateD.ExportVector3ListAsString(m.GetJ()));
