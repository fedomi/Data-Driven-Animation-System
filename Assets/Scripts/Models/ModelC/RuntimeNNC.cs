...
public class RuntimeNNC : MonoBehaviour
{
    ...
    [SerializeField]
    private float currentFrame = 1000;
    ...
    private static string filePythonNamePath = "F:/ProyectosUnity/VanillaMotionMatching/VanillaMotionMatching/Assets/Scripts/Python/TrainedNNC.py";
    ...
    {/*
        if (currentTime > inputTimer)
        {
            ...
            MotionStateC m = new MotionStateC(numbers);

            Debug.Log("Applying pose to " + m.GetJ().Count + " joints.");
            b.ApplyPose(m.GetJ(), m.GetH());
            //Debug.Log("Receiving A: " + received_data);
            //Debug.Log("Applying pose: " + MotionStateC.ExportVector3ListAsString(m.GetJ()));
     ...
     private void FixedUpdate()
    {
        //if (Input.GetKeyDown(KeyCode.Space)) {
            ...
            /*
            string received_data = MotionStateC.ExportVector3ListAsString(msl[(int)(currentFrame+1)].GetJ());
            //Debug.Log("Received: " + received_data);
            string[] numbers = received_data.Split(',');
    ...
    private void FixedUpdate()
    {
        //if (Input.GetKeyDown(KeyCode.Space)) {
            if (A)
            {
                ...
                string[] numbers = received_data.Split(' ');
                //MotionStateC m = new MotionStateC(numbers);
                MotionStateC m = new MotionStateC();
                m.AddPose(numbers);
                ...
                m.SetTP(msl[(int)currentFrame + 1].GetTP());
                m.SetPhase(msl[(int)currentFrame + 1].GetPhase());
                //Debug.Log("Pose: " + MotionStateC.ExportVector3ListAsString(m.GetJ()));
