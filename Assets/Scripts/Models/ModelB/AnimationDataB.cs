...
public class AnimationDataB : MonoBehaviour
{
    #region Attributes
    ...
    private string DataFile = "MotionData_014.txt";
    ...
    private void ExportData() {
        ...
        {
            foreach (MotionStateB m in msl)
            {
                //string line = m.GetP() + "," + m.GetQ() + "," + MotionStateB.Vector3ListToString(m.GetJ()) + "," + MotionStateB.Vector3ListToString(m.GetH());
            ...
            new StreamWriter(@"Assets\AnimationFiles\" + TrajectoriesFile, true))
        {
            foreach (MotionStateB m in msl)
            {
                string line = MotionStateB.ExportVector3ListAsString(m.GetTP());
                if(line.Length > 0)
                    file.WriteLine(line);
