...
public class RuntimeNNE : MonoBehaviour
{

    #region Attributes

    private Body b;
    private MotionStateE m;
    private List<MotionStateE> msl;
    [SerializeField]
    ...
    private string DataFile = "MotionData_019.txt";
    ...
    private static string filePythonNamePath = "F:/ProyectosUnity/VanillaMotionMatching/VanillaMotionMatching/Assets/Scripts/Python/TrainedNNE.py";
    ...
    {/*
        if (currentTime > inputTimer)
        {
            ...
            MotionStateE m = new MotionStateE(numbers);

            Debug.Log("Applying pose to " + m.GetJ().Count + " joints.");
            b.ApplyPose(m.GetJ(), m.GetH());
            //Debug.Log("Receiving A: " + received_data);
            //Debug.Log("Applying pose: " + MotionStateE.ExportVector3ListAsString(m.GetJ()));
     ...
     private void FixedUpdate()
    {
        //if (Input.GetKeyDown(KeyCode.Space)) {
            ...
            /*
            string received_data = MotionStateE.ExportVector3ListAsString(msl[(int)(currentFrame+1)].GetJ());
            //Debug.Log("Received: " + received_data);
            string[] numbers = received_data.Split(',');
   ...
   // b.ApplyPose(m.GetJ());
    ...
    private void FixedUpdate()
    {
        ...
        if (A)
        {
            ...
            string[] numbers = received_data.Split(' ');
            //MotionStateE m = new MotionStateE(numbers);
            MotionStateE m = new MotionStateE();
            m.AddPose(numbers);
            ...
            //Debug.Log("Pose: " + MotionStateE.ExportVector3ListAsString(m.GetJ()));
    ...
    private void DrawDebugLines(MotionStateE m)
    {
    ...
    private void ImportData()
    {
        ...
        using (StreamReader file = new StreamReader(@"Assets\AnimationFiles\" + DataFile))
        {
            ...
            while ((ln = file.ReadLine()) != null)
            {
                ...
                /*if(line.Length == 58)
                {
                    MotionStateE ms = new MotionStateE(line);
