...
public class AnimationDataE : MonoBehaviour
{
    #region Attributes
    ...
    private string DataFile = "MotionData_020.txt";
    ...
    private void DrawDebugLines(MotionStateE m) {
        ...
        if (drawTrajectoryLines) {
            ...
            //Debug.Log(MotionStateE.Vector2ListToString(tps));
    ...
    private void ExportData() {
        ...
        {
            foreach (MotionStateE m in msl)
            {
                //string line = m.GetP() + "," + m.GetQ() + "," + MotionStateE.Vector3ListToString(m.GetJ()) + "," + MotionStateE.Vector3ListToString(m.GetH());
            ...
            new StreamWriter(@"Assets\AnimationFiles\" + TrajectoriesFile, true))
        {
            foreach (MotionStateE m in msl)
            {
                string line = MotionStateE.ExportVector2ListAsString(m.GetTP());
                if(line.Length > 0)
                    file.WriteLine(line);
