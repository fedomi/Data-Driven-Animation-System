...
def readFile():
    N = 0
    file = open("F:\ProyectosUnity\VanillaMotionMatching\VanillaMotionMatching\Assets\Scripts\Python\MotionData_019.txt", "r")
    X = []
    j = 0
    for line in file:
        line_array = line.split(",")[1:]
        #print(len(line_array))
        if(len(line_array) == 174):
            #print(len(line_array))
            line_floats = []
            for i in line_array:
...
x_pose = [X[i][0:57] for i in range(0, len(X)-1)]
y_pose = [X[i][57:133] for i in range(0, len(X)-1)]
...
'''
mlreg = MLPRegressor(solver='adam', alpha=1e-5, max_iter = 250, verbose = 10, tol = 1e-10,
                     hidden_layer_sizes=(512, 512), random_state=1, n_iter_no_change = 30)
...
filename = 'F:\ProyectosUnity\VanillaMotionMatching\VanillaMotionMatching\Assets\Scripts\Python\Poses2RotationsNN.pkl'
